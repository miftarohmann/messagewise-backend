...
class PDFGenerator {
    ...
    addHeader(report) {
        ...
        this.doc
            ...
            .text(`Period: ${this.formatDate(report.period.start)} - ${this.formatDate(report.period.end)}`, 50, 145);
        // Account info
        this.doc
            .fontSize(10)
    ...
    addSummary(report) {
        ...
        const summaryData = [
            {
                label: 'Total Cost',
                value: (0, helpers_1.formatCurrency)(summary.totalCost),
            ...
            },
            {
                ...
                value: (0, helpers_1.formatCurrency)(summary.potentialSavings),
        ...
        this.doc
            ...
            .text(`Paid Messages: ${summary.paidMessages.toLocaleString()}`, 200, this.y)
            .text(`Free Ratio: ${(0, helpers_1.round)((summary.freeMessages / summary.totalMessages) * 100, 1)}%`, 350, this.y);
        this.y += 30;
        this.addDivider();
    }
    /**
     * Add summary box
     */
    addSummaryBox(x, y, width, height, data) {
        // Box background
        this.doc
    ...
    addCategoryBreakdown(report) {
        ...
        breakdown.forEach((item) => {
            ...
            this.doc
                ...
                .text((0, helpers_1.formatCurrency)(item.cost), 300, this.y, {
                width: colWidths[2],
                align: 'right',
            })
                .text(`${(0, helpers_1.round)(item.percentage, 1)}%`, 400, this.y, {
                width: colWidths[3],
                align: 'right',
            });
            this.y += 20;
     ...
     * Add trend item
     */
    addTrendItem(x, y, label, trend) {
        const arrow = trend.trend === 'up' ? '↑' : trend.trend === 'down' ? '↓' : '→';
        const color = label === 'Savings'
            ? trend.trend === 'up'
                ? COLORS.success
                : COLORS.danger
            : trend.trend === 'up'
        ...
        this.doc
            .fontSize(12)
            .fillColor(COLORS.text)
            .text(typeof trend.current === 'number' && trend.current > 1000
            ? trend.current.toLocaleString()
            : (0, helpers_1.formatCurrency)(trend.current), x, y + 15);
        this.doc
            .fontSize(10)
            .fillColor(color)
            .text(`${arrow} ${(0, helpers_1.round)(Math.abs(trend.changePercentage), 1)}%`, x, y + 32);
    }
    /**
    ...
    addRecommendations(recommendations) {
        ...
        recommendations.slice(0, 5).forEach((rec, index) => {
            ...
            this.doc
                ...
                .text(`Potential Savings: ${(0, helpers_1.formatCurrency)(rec.potentialSavings)}`, 60, this.y + 40);
            this.y += 65;
        });
        this.addDivider();
    }
    ...
    addDailyBreakdown(report) {
        ...
        report.dailyData.slice(-14).forEach((day) => {
            ...
            this.doc
                ...
                .text((0, helpers_1.formatCurrency)(day.totalCost), 370, this.y, {
                width: colWidths[4],
                align: 'right',
            });
            this.y += 18;
    ...
    addFooter() {
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            this.doc
                .fontSize(8)
                .fillColor(COLORS.lightText)
                .text(`Page ${i + 1} of ${pageCount}`, 50, this.doc.page.height - 40, { align: 'center', width: this.doc.page.width - 100 })
                .text('Generated by MessageWise - messagewise.com', 50, this.doc.page.height - 30, { align: 'center', width: this.doc.page.width - 100 });
        }
    }
    /**
     * Add section title
     ...
     */
    formatDate(date) {
        return new Date(date).toLocaleDateString(this.options.language === 'id' ? 'id-ID' : 'en-US', {
            year: 'numeric',
            month: 'short',
            day: 'numeric',
        });
