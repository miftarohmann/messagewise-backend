...
class EmailService {
    ...
    initialize() {
        ...
        if (!smtpHost || !smtpUser || !smtpPass) {
            logger_1.default.warn('Email service disabled: SMTP credentials not configured');
        ...
        try {
            ...
            this.isEnabled = true;
            logger_1.default.info('Email service initialized', { host: smtpHost });
        }
        catch (error) {
    ...
    async sendEmail(options) {
        if (!this.isEnabled || !this.transporter) {
            logger_1.default.warn('Email not sent: service disabled', { to: options.to });
            return false;
        }
        try {
            ...
            logger_1.default.info('Email sent successfully', { to: options.to, subject: options.subject });
            return true;
        }
        catch (error) {
     ...
     * Daily summary email
     */
    async sendDailySummaryEmail(to, name, data) {
     ...
     * Cost alert email
     */
    async sendCostAlertEmail(to, name, data) {
     ...
     */
    async sendPaymentConfirmationEmail(to, name, data) {
     ...
     */
    async sendSubscriptionExpiringEmail(to, name, data) {
...
<body>
  <div class="container">
    ...
    getDailySummaryTemplate(name, data) {
    ...
    getCostAlertTemplate(name, data) {
        const formatIDR = (amount) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(amount);
        const content = `
    ...
    getPaymentConfirmationTemplate(name, data) {
    ...
    getSubscriptionExpiringTemplate(name, data) {
        const content = `
