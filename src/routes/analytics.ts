...
import { validate, analyticsValidation } from '../middleware/validator';
...
import { SavingsPredictor } from '../services/analyzer/savingsPredictor';
import { startOfDay, endOfDay, startOfMonth, endOfMonth, daysAgo } from '../utils/helpers';
...
router.get(
  '/summary',
  authenticate,
  validate(analyticsValidation.summary),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const end = endDate ? new Date(endDate as string) : endOfDay();
      ...
      const dailyMap = new Map<string, { date: string; messages: number; cost: number }>();
      messages.forEach(m => {
...
router.get(
  ...
  authenticate,
  analysisRateLimiter,
  validate(analyticsValidation.breakdown),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      logger.error('Get cost breakdown failed', { error });
      res.status(500).json({
        success: false,
        error: {
          code: ERROR_CODES.INTERNAL_ERROR,
...
router.get(
  '/:accountId/trends',
  authenticate,
  validate(analyticsValidation.trends),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { accountId } = req.params;
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Generate forecast
      const predictor = new SavingsPredictor();
      ...
      const forecast = predictor.generateForecast(historicalData, parseInt(months as string));
      ...
      logger.error('Get forecast failed', { error });
      res.status(500).json({
        success: false,
        error: {
          code: ERROR_CODES.INTERNAL_ERROR,
    ...
    try {
      ...
      const analyses = await db.costAnalysis.findMany({
        where: {
          accountId,
          periodType: 'DAILY',
          periodStart: { gte: daysAgo(30) },
        },
        orderBy: { periodStart: 'asc' },
      });

      // Extract category data
      const categoryData: Record<string, Array<{ date: string; count: number; cost: number }>> = {
        MARKETING: [],
        UTILITY: [],
        AUTHENTICATION: [],
        SERVICE: [],
      ...
      analyses.forEach((a) => {
        const breakdown = a.breakdown as Array<{ category: string; count: number; cost: number }>;
